...
import { Filter, X, Calendar, Flame, DollarSign, ChevronDown } from 'lucide-react';
...
interface ProfessionalFiltersProps {
  ...
  selectedProfessions: ProfessionType[];
}

export const ProfessionalFilters = ({ filters, onFiltersChange, selectedProfessions }: ProfessionalFiltersProps) => {
  const { t } = useLanguage();
  const [isOpen, setIsOpen] = useState(false);
  const [localFilters, setLocalFilters] = useState<FiltersType>(filters);
  const [priceRange, setPriceRange] = useState([0, 200000]);

  // Determine which skills to show based on selected professions
  const getRelevantSkills = () => {
    if (selectedProfessions.length === 0) {
      return [...MUSIC_GENRES.slice(0, 4), ...BARTENDER_SKILLS.slice(0, 4)];
    }
    
    const skills: string[] = [];
    if (selectedProfessions.includes('dj')) skills.push(...MUSIC_GENRES);
    if (selectedProfessions.includes('bartender')) skills.push(...BARTENDER_SKILLS);
    if (selectedProfessions.includes('photographer')) skills.push(...PHOTOGRAPHER_SKILLS);
    if (selectedProfessions.includes('security')) skills.push(...SECURITY_SKILLS);
  ...
  const toggleSkill = (skill: string) => {
    ...
    // Check if it's a genre or skill
    if (MUSIC_GENRES.includes(skill)) {
      const updated = currentGenres.includes(skill)
        ? currentGenres.filter(g => g !== skill)
        : [...currentGenres, skill];
  ...
  const isSkillSelected = (skill: string) => {
    return (localFilters.genres || []).includes(skill) || (localFilters.skills || []).includes(skill);
  };

  const activeFilterCount = [
    (filters.genres?.length || 0) + (filters.skills?.length || 0),
